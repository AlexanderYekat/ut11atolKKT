...
Процедура КомандаПечатьЧекаНаККТ(Команда)
	...
	полнСтруктЧека = ПодготовитьДанныеДляЧекаНаСервере(ОписаниеВозращОшибк, былаОшибка);
...
функция СформироватьСтруктуруТестовогоЧека(ЭтоЧекВозрата, ДобавлятьМакри, ЭтоПлатеж = ЛОЖЬ)
	...
	////списПараметровЧека.Вставить("СНО", "Патент");
...
функция ПодготовитьМассивТоваровПоДокументу(докумСсылка)
	...
	//списПараметровЧека.Вставить("Наличными", 1);
	ЭтоЧекВозрата = ЛОЖЬ;
	Если ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") тогда
		...
		конецЕсли;		
	иначеЕсли ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") или 
	...
	//списПараметровЧека.Вставить("ЗачетАванса", 0);
	списПараметровЧека.Вставить("Кассир", ПолучитьИмяКассира());
	списПараметровЧека.Вставить("ЭтоЧекВозврата", ЭтоЧекВозрата);
	//позиции
	массивЧека = Новый Массив;                  
	//массивЧека.Добавить(Новый Структура);
...
&НаСервере
Функция ПодготовитьДанныеДляЧекаНаСервере(описаниеОшибки = "", былаОшибка = ЛОЖЬ)
	былаОшибка = ЛОЖЬ;
	...
	Если ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") или
		...
		полнСтруктЧека = ПодготовитьМассивТоваровПоДокументу(Объект.РеквизитДокумент);
	иначеЕсли ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.ЧекККМ") тогда
		Сообщить("ЧекККМ");
		описаниеОшибки = "пока не доделано печать чека из рабочего места кассира";
		былаОшибка = ИСТИНА;
	иначеЕсли ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
		Сообщить("РеализацияТоваровУслуг");	
		описаниеОшибки = "печать чека из реализации не реализовано - не было в первоначальном ТЗ";
		былаОшибка = ИСТИНА;
	иначе
