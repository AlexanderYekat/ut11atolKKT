&НаКлиенте
Перем РезультатЗакрытияФормы;
&НаКлиенте
Перем ИдетЗакрытиеФормыСПробитиемЧека;

&НаКлиенте
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	// Вставить содержимое обработчика.
	Если НЕ ИдетЗакрытиеФормыСПробитиемЧека тогда
	конецЕсли;
КонецПроцедуры //ПередЗакрытием

&НаКлиенте
Процедура КомандаПробитьЧек(Команда)
	ИдетЗакрытиеФормыСПробитиемЧека = ИСТИНА;
	РезультатЗакрытияФормы.Вставить("Результат", ИСТИНА);
	РезультатЗакрытияФормы.Вставить("ТаблицаОплат", ПолучитьСтруктруруОплатИзПолейФормы());
	Закрыть(РезультатЗакрытияФормы);
КонецПроцедуры //КомандаПробитьЧек

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	РеквизитJSONЧека = Параметры.ПараметрJSONЧека;	
КонецПроцедуры //ПриСозданииНаСервере

&НаКлиенте
Функция ПолучитьСтруктруруОплатИзПолейФормы()
	СтруктрураОплат = Новый Структура;
	СтруктрураОплат.Вставить("Наличными", РеквизитСуммаНаличными);
	СтруктрураОплат.Вставить("Безналичными", РеквизитСуммаБезналичными);
	СтруктрураОплат.Вставить("ЗачетАванса", РеквизитСуммаЗачетАванса);
	СтруктрураОплат.Вставить("Кредит", РеквизитСуммаРассрочки);
	Возврат СтруктрураОплат;
конецФункции //ПолучитьСтруктруруОплатИзПолейФормы

&НаКлиенте
процедура ЗполнитьПоляОплат(СтруктрураОплат)
	РеквизитСуммаНаличными = 0;
	РеквизитСуммаБезналичными = 0;
	РеквизитСуммаЗачетАванса = 0;
	РеквизитСуммаРассрочки = 0;
	Если СтруктрураОплат.Свойство("Наличными") тогда
		РеквизитСуммаНаличными = СтруктрураОплат.Наличными;
	конецЕсли;
	Если СтруктрураОплат.Свойство("Безналичными") тогда
		РеквизитСуммаБезналичными = СтруктрураОплат.Безналичными;
	конецЕсли;
	Если СтруктрураОплат.Свойство("ЗачетАванса") тогда
		РеквизитСуммаЗачетАванса = СтруктрураОплат.ЗачетАванса;
	конецЕсли;
	Если СтруктрураОплат.Свойство("Кредит") тогда
		РеквизитСуммаРассрочки = СтруктрураОплат.Кредит;
	конецЕсли;
конецПроцедуры //ЗполнитьПоляОплат

&НаКлиенте
Процедура ПриОткрытии(Отказ)	
	//ФормаОжиданияОтвета = ПолучитьФорму("ОбщаяФорма.ФормаОжиданияОтветаОтККТ", ПарамДляФормы, ФормаВладелец);
	//имяОткрываемойФормы = Объект.РеквизитОбъектаИмяОбработки+".Форма.Форма";
	//Сообщить("При открытии печатня форма");
	ИдетЗакрытиеФормыСПробитиемЧека = ЛОЖЬ;
	РезультатЗакрытияФормы = Новый Структура;
	РезультатЗакрытияФормы.Вставить("Результат", ЛОЖЬ);
	ответСтруктураИзJSON = Новый Структура;
	чекСтруктураИзJSONЗадание = ПолучитьСтруктуруИЗJSON(РеквизитJSONЧека);
	всеПараметрыЧека = ПреобразоватьВЧитабельныйВидСтрктуруЧека(РеквизитНазваниеФирмы, "", 
												чекСтруктураИзJSONЗадание, ответСтруктураИзJSON);
	ПроверитьИИсправитьСтрокиОплат(всеПараметрыЧека.ТаблицаОплат, всеПараметрыЧека.СуммаЧека);
	//ПечатьЧекаККТСервер.Печать(РеквизитПечатнаяФормаЧека, всеПараметрыЧека);
	//Сообщить("Печать1");
	Печать(РеквизитПечатнаяФормаЧека, всеПараметрыЧека);
	//Сообщить("Печать2");
	ЗполнитьПоляОплат(всеПараметрыЧека.ТаблицаОплат);
	//Сообщить("ПечатьКон");
КонецПроцедуры //ПриОткрытии

&НаКлиенте
Процедура ПолеСуммаПриИзменении(Элемент)
	ПолучитьСтруктруруОплатИзПолейФормы();
	ответСтруктураИзJSON = Новый Структура;
	чекСтруктураИзJSONЗадание = ПолучитьСтруктуруИЗJSON(РеквизитJSONЧека);
	всеПараметрыЧека = ПреобразоватьВЧитабельныйВидСтрктуруЧека(РеквизитНазваниеФирмы, "", 
												чекСтруктураИзJSONЗадание, ответСтруктураИзJSON);
	таблицаОплатНовая = ПолучитьСтруктруруОплатИзПолейФормы();
	ПроверитьИИсправитьСтрокиОплат(таблицаОплатНовая, всеПараметрыЧека.СуммаЧека);
	ЗполнитьПоляОплат(таблицаОплатНовая);
	Для Каждого стр из таблицаОплатНовая цикл
		всеПараметрыЧека.ТаблицаОплат.Вставить(стр.Ключ, стр.Значение);
	конецЦикла;	
	//ПечатьЧекаККТСервер.Печать(РеквизитПечатнаяФормаЧека, всеПараметрыЧека);
	Печать(РеквизитПечатнаяФормаЧека, всеПараметрыЧека);
КонецПроцедуры //ПолеСуммаПриИзменении

//&НаКлиенте
//процедура ПроверитьИИсправитьСтрокиОплат(СуммаЧека, ТаблицаОплат)
//	всегоСуммаОплат = 0;
//	всегоСуммаОплатИсключаяНаличные = 0;
//	Для Каждого стр из ТаблицаОплат цикл
//		всегоСуммаОплат = всегоСуммаОплат + стр.Значение;
//		Если стр.Ключ <> "Наличными" тогда
//			всегоСуммаОплатИсключаяНаличные = всегоСуммаОплатИсключаяНаличные + стр.Значение;
//		конецЕсли;
//	конецЦикла;
...
//				Продолжить;
//			конецЕсли;
//			Если СуммаНаКоторуюНадоУменьшить > стр.Значение тогда
//				стр.Значение = 0;
//				СуммаНаКоторуюНадоУменьшить = СуммаНаКоторуюНадоУменьшить - стр.Значение;
//			иначе
//				стр.Значение = стр.Значение - СуммаНаКоторуюНадоУменьшить;
...
//		ТаблицаОплат.Наличными = ТаблицаОплат.Наличными + (СуммаЧека - всегоСуммаОплат);
//	конецЕсли;
//	Если всегоСуммаОплат > СуммаЧека тогда
//		ТаблицаОплат.Вставить("Сдача", всегоСуммаОплат - СуммаЧека);
//	конецЕсли;	
