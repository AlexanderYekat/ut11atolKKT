...
функция ПолчитьПолноеИмяФайлаОбработкиДляПечатиЧека(имяОбработки)
	//"C:\docum\fl\utAtol\НастройкаСвязисККТ_fl.epf"
	//Сообщить("Процедура ПолчитьИмяОбработкиДляПечатиЧека");									
	имяОбработкиЛок = имяОбработки+".epf";
	ВыбФайл = Новый Файл("c:\share\"+имяОбработкиЛок);
	Если ВыбФайл.Существует() тогда
		возврат "c:\share\"+СокрЛП(имяОбработкиЛок);
	конецЕсли;
	возврат "C:\docum\fl\utAtol\"+СокрЛП(имяОбработкиЛок);
...
Функция ПечатьВызватьКомандуНаККТВместоВстроеннойВКонфигурацию(Команда, ОповещениеПриЗавершенииПечатиЧека, ФормаДокумента,
	...
	печатьЧекаИзВнешнегоФайла = ИСТИНА;																	
	...
	//процедура обрабатывающая результат печти чека - она находится в модуле формы документа
	...
	допПараметры.Вставить("ОбработкаВОбъектеМетеданных", ЛОЖЬ);
	имяФайлаОбработкиПечатиЧека = "ПечатьЧекаККТВнеКонфигурации";
	допПараметры.Вставить("ИмяОбработкиДляПечтиЧековВКонфигарции", "ВнешняяОбработкаПечатьЧекаККТ");																
	
	Если (МенеджерОборудованияВызовСервера.ОбработкаПрисутсвуетВКонфигурации(допПараметры.ИмяОбработкиДляПечтиЧековВКонфигарции)) 
			и НЕ (печатьЧекаИзВнешнегоФайла) тогда
		допПараметры.Вставить("ОбработкаВОбъектеМетеданных", ИСТИНА);
		ОбработчикОкончанияПомещения(ИСТИНА, "", "", допПараметры);
		возврат ИСТИНА;
		//если вернём ложь, то продолжится стандартная обработка печати чека конфигурации
	конецЕсли;
	имяОбработкиДляПечатиЧека = ПолчитьПолноеИмяФайлаОбработкиДляПечатиЧека(имяФайлаОбработкиПечатиЧека);
...
Процедура ОбработчикОкончанияПомещения(Результат, Адрес, 
	...
	Если НЕ Результат Тогда
		...
		возврат;
	КонецЕсли;
	имяОбр = ДополнительныеПараметры.ИмяОбработкиДляПечтиЧековВКонфигарции;
	ПрефиксМетаднных = "Обработка";
	Если НЕ ДополнительныеПараметры.ОбработкаВОбъектеМетеданных тогда
		имяОбр = МенеджерОборудованияВызовСервера.ПолучитьИмяВнешнейОбработки(Адрес);	
		ПрефиксМетаднных = "ВнешняяОбработка";
	конецЕсли;	
	...
	ФормаОжиданияОтвета = ПолучитьФорму(ПрефиксМетаднных+"."+имяОбр+".Форма.Форма", ПарамДляОбработкиПечатиЧека, ДополнительныеПараметры.ФормаДокум);
