...
Функция ПодготовитьДанныеДляЧекаНаСервере(кассир = "", описаниеОшибки = "", былаОшибка = ЛОЖЬ)
	...
	иначеЕсли ТипЗнч(Объект.РеквизитДокумент) = Тип("ДокументСсылка.ЧекККМ") тогда
		Сообщить("ЧекККМ");
...
функция  ПодготовитьМассивТоваровПоДокументуОтгрузки(докумСсылка, кассир = "")
	...
	Если ТипЗнч(докумСсылка) = Тип("ДокументСсылка.ЧекККМ") тогда
		...
		СуммаОплатыСертификатом =  Объект.ПодарочныеСертификаты.Итог("Сумма");
	...
	конецЕсли;	
	Если ТипЗнч(докумСсылка) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") тогда
	...
	массивЧека = ПодготовитьМассивТоваровПоТабличнойЧастиДокументаОтгрузки(массивЧека, докумСсылка, обдок, типРасчета);
...
функция ПодготовитьМассивТоваровПоДокументуОплаты(докумСсылка, кассир = "")
	...
	если всегоОснов > 0 тогда
		для счОсн = 0 по всегоОснов-1 цикл
			...
			массивЧека = ПодготовитьМассивТоваровПоТабличнойЧастиДокументаОтгрузки(массивЧека, основ, основОб, типРасчета);			
			...
			//	списПараметровПозиции.Вставить("Цена", стрТов.Цена);
...
&НаСервере
функция ПодготовитьМассивТоваровПоТабличнойЧастиДокументаОтгрузки(массивЧека, докумСсылка, докумОбъект, типРасчета = "")
	Перем докумРеалиЧекаОьъект;
	Если НЕ ЗначениеЗаполнено(докумОбъект) тогда
		докумРеалиЧекаОьъект = докумСсылка.ПолучитьОбъект();
	иначе
		докумРеалиЧекаОьъект = докумОбъект;
	конецЕсли;
